...
export class ProductCategoryNewComponent implements OnInit {
  ...
  getCategories() {
    this.categoryHttp.listWithParams({all: 1})
      .subscribe(response => {
  ...
  /**
   * Para evitar a duplicação de ids de categoria
   */
  private mergeCategories(): number[] {
    ...
    const newCategoriesId = this.categoriesId.filter((category) => {
      // indexOf: se ele não encontrar o índice ele retorna -1
      return categoriesId.indexOf(category) == -1; // retorna o índice do elemento dentro do array
    });
